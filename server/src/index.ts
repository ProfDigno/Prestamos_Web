...
async function createOperation(req:Request,res:Response,type:'PRESTAMO'|'VENTA_FINANCIADA'){
  ...
  const id=await transaction(async db=>{
    ...
    const opId=op.rows[0].idoperacion_financiera;if(broker){const commission=calculateBrokerCommission(calc.capital,broker.porcentaje_comision);await db.query('INSERT INTO operacion_corredor(fk_idoperacion_financiera,fk_idcorredor,porcentaje_comision,monto_comision,creado_por) VALUES($1,$2,$3,$4,$5)',[opId,broker.idcorredor,broker.porcentaje_comision,commission,req.user!.id]);}const plan=await db.query('INSERT INTO plan_pago (fk_idoperacion_financiera,frecuencia,creado_por) VALUES ($1,$2,$3) RETURNING idplan_pago',[opId,data.frecuencia,req.user!.id]);
...
app.get('/api/corredores/activos',requireAuth,asyncRoute(async(_req,res)=>simpleList(res,'SELECT idcorredor,nombre_completo,porcentaje_comision FROM corredor WHERE activo ORDER BY nombre_completo')));
app.get('/api/corredores',requirePermission('ADMINISTRAR'),asyncRoute(async(_req,res)=>simpleList(res,`SELECT c.*,COUNT(DISTINCT oc.fk_idoperacion_financiera) FILTER (WHERE oc.activo)::integer AS operaciones FROM corredor c LEFT JOIN operacion_corredor oc ON oc.fk_idcorredor=c.idcorredor GROUP BY c.idcorredor ORDER BY c.nombre_completo`)));
app.get('/api/corredores/:id/operaciones',requirePermission('ADMINISTRAR'),asyncRoute(async(req,res)=>{const broker=(await pool.query('SELECT idcorredor,nombre_completo,cedula,telefono,email,porcentaje_comision,activo FROM corredor WHERE idcorredor=$1',[req.params.id])).rows[0];if(!broker)return res.status(404).json({error:'Corredor no encontrado'});const operations=(await pool.query(`SELECT o.idoperacion_financiera,o.tipo,o.fecha_inicio,o.monto_capital,o.monto_interes,o.monto_total,o.estado,c.nombre_completo AS cliente,oc.porcentaje_comision,oc.monto_comision,(o.estado='CANCELADA') AS cancelada FROM operacion_corredor oc JOIN operacion_financiera o ON o.idoperacion_financiera=oc.fk_idoperacion_financiera JOIN cliente c ON c.idcliente=o.fk_idcliente WHERE oc.fk_idcorredor=$1 AND oc.activo ORDER BY o.fecha_inicio DESC,o.idoperacion_financiera DESC`,[req.params.id])).rows;const vigentes=operations.filter((item)=>item.estado!=='CANCELADA');const capitalTotal=vigentes.reduce((sum,item)=>sum.plus(item.monto_capital),new Decimal(0));const comisionTotal=vigentes.reduce((sum,item)=>sum.plus(item.monto_comision),new Decimal(0));res.json({corredor:broker,resumen:{operaciones:operations.length,capital_total:capitalTotal.toFixed(2),comision_total:comisionTotal.toFixed(2)},operaciones:operations});}));
